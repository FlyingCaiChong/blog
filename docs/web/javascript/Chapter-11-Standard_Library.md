...
::: tip 翻译
您已经看到了本书中使用的 `console.log()` 函数：在网络浏览器中，它在浏览器开发人员工具窗格的“控制台”选项卡中打印一个字符串，这在调试时非常有帮助。 在 Node 中，`console.log()` 是一个通用输出函数，并将其参数打印到进程的 `stdout` 流，通常在终端窗口中作为程序输出向用户显示。
:::
...
::: tip 翻译
除了 `console.log()` 之外，控制台 API 还定义了许多有用的函数。 该 API 不属于任何 ECMAScript 标准，但它受到浏览器和 Node 的支持，并且已在 https://console.spec.whatwg.org 上正式编写和标准化。
:::
...
::: tip 翻译
这是最著名的控制台功能。 它将其参数转换为字符串并将其输出到控制台。 它在参数之间包含空格，并在输出所有参数后开始新行。
:::
...
::: tip 翻译
这些函数几乎与 `console.log()` 相同。 在 Node 中，`console.error()` 将其输出发送到 `stderr` 流而不是 `stdout` 流，但其他函数是 `console.log()` 的别名。 在浏览器中，由每个函数生成的输出消息可能会以指示其级别或严重性的图标为前缀，并且开发人员控制台还可以允许开发人员按级别过滤控制台消息。
:::
...
::: tip 翻译
如果第一个参数为真（即，如果断言通过），则该函数不执行任何操作。 但是，如果第一个参数是 `false` 或另一个虚假值，则剩余的参数将被打印，就好像它们已传递给带有“断言失败”前缀的 `console.error()` 一样。 请注意，与典型的 `assert()` 函数不同，`console.assert()` 在断言失败时不会引发异常。
:::
...
::: tip 翻译
如果可能的话，此函数会清除控制台。 当 Node 将其输出显示到终端时，这适用于浏览器和 Node。 但是，如果 Node 的输出已重定向到文件或管道，则调用此函数无效。
:::
...
::: tip 翻译
这个函数是一个非常强大但鲜为人知的功能，用于生成表格输出，它在需要生成汇总数据的输出的 Node 程序中特别有用。 `console.table()` 尝试以表格形式显示其参数（尽管，如果它不能这样做，它会使用常规的 `console.log()` 格式显示它）。 当参数是相对较短的对象数组，并且数组中的所有对象都具有相同（相对较小）的属性集时，此方法效果最佳。 在这种情况下，数组中的每个对象都被格式化为表的一行，每个属性都是表的一列。 您还可以传递属性名称数组作为可选的第二个参数来指定所需的列集。 如果传递一个对象而不是对象数组，则输出将是一个表，其中一列用于属性名称，一列用于属性值。 或者，如果这些属性值本身就是对象，则它们的属性名称将成为表中的列。
:::
...
::: tip 翻译
这个函数像 `console.log()` 一样记录它的参数，此外，它的输出后面还有堆栈跟踪。 在 Node 中，输出转到 `stderr` 而不是 `stdout`。
:::
...
::: tip 翻译
该函数接受一个字符串参数并记录该字符串，后跟使用该字符串调用它的次数。 这在调试事件处理程序时非常有用，例如，如果您需要跟踪事件处理程序被触发的次数。
:::
...
::: tip 翻译
该函数将其参数打印到控制台，就像它们已传递给 `console.log()` 一样，然后设置控制台的内部状态，以便所有后续控制台消息（直到下一个 `console.groupEnd()` 调用） 将相对于刚刚打印的消息进行缩进。 这允许通过缩进将一组相关消息直观地分组。 在 Web 浏览器中，开发人员控制台通常允许将分组消息作为一个组折叠和展开。 `console.group()` 的参数通常用于为组提供解释性名称。
:::
...
::: tip 翻译
此函数的工作方式类似于 `console.group()`，不同之处在于，在网络浏览器中，该组将默认“折叠”，并且除非用户单击展开该组，否则它包含的消息将被隐藏。 在 Node 中，此函数是 `console.group()` 的同义词。
:::
...
::: tip 翻译
该函数不带参数。 它不会产生自己的输出，但会结束由最近调用 `console.group()` 或 `console.groupCollapsed()` 引起的缩进和分组。
:::
...
::: tip 翻译
该函数采用单个字符串参数，记录使用该字符串调用它的时间，并且不产生任何输出。
:::
...
::: tip 翻译
该函数采用字符串作为其第一个参数。 如果该字符串之前已传递给 `console.time()`，则它会打印该字符串，后跟自 `console.time()` 调用以来经过的时间。 如果 `console.timeLog()` 有任何其他参数，它们将被打印，就像它们已传递给 `console.log()` 一样。
:::
...
::: tip 翻译
该函数采用单个字符串参数。 如果该参数之前已传递给 `console.time()`，那么它会打印该参数和经过的时间。 调用 `console.timeEnd()` 后，如果不再次调用 `console.time()`，则调用 `console.timeLog()` 不再合法。
:::
...
::: tip 翻译
像 `console.log()` 这样打印参数的控制台函数有一个鲜为人知的功能：如果第一个参数是包含 `%s`、`%i`、`%d`、`%f` `%o`、`%O` 或 `%c`的字符串，则第一个参数被视为格式字符串，并且后续参数的值将替换到字符串中以代替两个字符的 `%` 序列。
:::
...
::: tip 翻译
参数被视为对象，并显示属性名称和值。 （在 Web 浏览器中，这种显示通常是交互式的，用户可以展开和折叠属性来探索嵌套的数据结构。）`%o` 和 `%O` 都显示对象详细信息。 大写变体使用依赖于实现的输出格式，该格式被认为对软件开发人员最有用。
:::
...
::: tip 翻译
在 Web 浏览器中，该参数被解释为 CSS 样式字符串，并用于设置后面的任何文本的样式（直到下一个 `%c` 序列或字符串末尾）。 在 Node 中，`%c` 序列及其相应的参数被简单地忽略。
:::
...
::: tip 翻译
请注意，通常不需要在控制台函数中使用格式字符串：通过简单地将一个或多个值（包括对象）传递给函数并允许实现以有用的方式显示它们，通常很容易获得合适的输出 。 例如，请注意，如果将 `Error` 对象传递给 `console.log()`，它会自动与其堆栈跟踪一起打印。
:::
